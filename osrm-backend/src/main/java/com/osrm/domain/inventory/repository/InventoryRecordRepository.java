...
public interface InventoryRecordRepository extends JpaRepository<InventoryRecord, Long>, JpaSpecificationExecutor<InventoryRecord> {
    ...
    Page<InventoryRecord> findByUserId(Long userId, Pageable pageable);

    // 按用户ID和状态查询
    Page<InventoryRecord> findByUserIdAndStatus(Long userId, InventoryRecord.InventoryStatus status, Pageable pageable);

    // 按状态查询（用于管理员查看待审批列表）
    Page<InventoryRecord> findByStatus(InventoryRecord.InventoryStatus status, Pageable pageable);
    ...
    // 条件查询（管理员使用）
    @Query("SELECT i FROM InventoryRecord i WHERE " +
