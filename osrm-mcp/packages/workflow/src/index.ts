...
const tools = [
  // ── 订购类 ─────────────────────────────────────────────────────────────────
  {
    ...
    description:
      '提交软件订购申请。需要指定软件包版本、关联的业务系统和使用场景描述。申请提交后进入审批流程。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        subscription_id: { type: 'number', description: '订购记录ID' },
      },
  ...
  // ── 软件管理类 ──────────────────────────────────────────────────────────────
  {
    ...
    description:
      '创建新软件包（草稿状态）。packageKey 是全局唯一的英文标识符，创建后不可修改。',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        package_key: {
          type: 'string',
          description: '全局唯一英文标识符（字母/数字/连字符，创建后不可修改）',
        },
        software_type: {
          type: 'string',
          description: '软件类型',
  ...
  },
  {
    name: 'add_version',
    description:
      '为软件包添加新版本，指定存储后端。版本创建后状态为 DRAFT，需上传制品后才能发布。',
    inputSchema: {
      type: 'object',
      properties: {
        package_id: { type: 'number', description: '软件包ID' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        keyword: { type: 'string', description: '按系统名称搜索（可选）' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description:
      '检查指定业务系统的软件使用合规情况：列出该系统订购并使用了哪些软件，标注是否已审批通过。',
    inputSchema: {
      type: 'object',
      properties: {
        system_id: { type: 'number', description: '业务系统ID' },
      },
      required: ['system_id'],
    },
  },
...
const server = new Server(
  { name: 'osrm-workflow', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
async function main() {
  ...
  console.error('[osrm-workflow] MCP server started');
}

main().catch((e) => {
  console.error('[osrm-workflow] Fatal error:', e);
  process.exit(1);
});
